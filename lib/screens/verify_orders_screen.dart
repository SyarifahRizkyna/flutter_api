...
class _VerifyOrdersScreenState extends State<VerifyOrdersScreen> {
  ...
  Future<void> fetchOrders() async {
    ...
    if (response.statusCode == 200) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Gagal mengambil data pesanan!')));
    }
  }
  ...
  Future<void> verifyTransaction(int transactionId) async {
    ...
    if (response.statusCode == 200) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Transaksi berhasil diverifikasi!')));
      fetchOrders(); // Refresh transaksi setelah diverifikasi
    } else {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Gagal memverifikasi transaksi!')));
    }
  }
      ...
      body: ListView.builder(
        ...
        itemBuilder: (context, index) {
          return ListTile(
            ...
            subtitle: Text('Antar jemput: ${_orders[index]['pickup'] ? 'Ya' : 'Tidak'}'),
            trailing: IconButton(
              icon: Icon(Icons.check),
              onPressed: () {
                verifyTransaction(_orders[index]['id']); // Verifikasi pesanan berdasarkan ID
              },
            ),
          );
        },
