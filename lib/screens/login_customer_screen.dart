  ...
  Future<void> loginCustomer() async {
    if (_formKey.currentState!.validate()) {
      final response = await http.post(
        Uri.parse('http://127.0.0.1:8000/login/customer'),
      ...
      if (response.statusCode == 200) {
        // Success
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Login pelanggan berhasil!')));
        // Lakukan navigasi atau aksi setelah login berhasil
      } else {
        // Error
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Gagal login pelanggan!')));
      }
    }
  }
